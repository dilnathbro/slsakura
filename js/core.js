// ============================================================
        ...
        // Expose core services for modular feature files that need them.
        window.db = db;
        window.auth = auth;
        window.state = state;
        ...
        const ADMIN_UIDS = new Set(['BuwRwihooZfRDG7mRryPK4OhsRD2']);

        auth.onAuthStateChanged(user => {
            // Only the configured administrator account receives admin privileges.
            state.adminLoggedIn = !!(user && ADMIN_UIDS.has(user.uid));
            updateUserUI();
            updateLoginFeatures();
        ...
        function updateLoginFeatures() {
            ...
            if (user) {
                // Keep the feature grid visible after login; only its actions become unlocked.
                if (loginFeatures) loginFeatures.classList.remove('hidden');
                if (userDashboard) userDashboard.classList.remove('hidden');
                updateUserDashboard();
                if (typeof window.loadMemberFeatureConfig === 'function') {
                    window.loadMemberFeatureConfig().then(() => {
                        if (typeof window.syncMemberUI === 'function') window.syncMemberUI();
                    });
                }
            } else {
                if (loginFeatures) loginFeatures.classList.remove('hidden');
        ...
        function loadResourcesPaginated(append = false) {
            // The site already keeps the resource collection in memory. For this
            // project size, one safe fetch + client-side Load More is more reliable
            // than a cursor query that can skip/duplicate records.
            if (pagination.isLoading) return;
            pagination.isLoading = true;
            if (!append) state.visibleCount = 12;
            ...
            db.ref('resources').once('value').then(snapshot => {
                ...
                pagination.totalCount = state.resources.length;
        ...
        function loadMoreResources() {
            // loadAllData() already keeps the complete resource list in memory.
            // Reveal the next page instead of issuing a second Firebase query.
            const total = getFilteredResources().length;
        ...
        function loadAllData(force = false) {
            // Prevent repeated startup retries and overlapping Firebase reads.
            if (dataLoadPromise && !force) return dataLoadPromise;
            const now = Date.now();
            if (!force && now - lastDataLoadAt < 350 && dataLoadPromise) return dataLoadPromise;
            ...
            dataLoadPromise = Promise.all([
                ...
                db.ref('advertisements').once('value'),
                db.ref('settings/buyMeACoffeeUrl').once('value')
            ]).then(([resourcesSnap, newsSnap, adsSnap, coffeeSnap]) => {
                state.resources = Object.entries(resourcesSnap.val() || {}).map(([id, value]) => ({ id, ...value }));
                state.news = Object.entries(newsSnap.val() || {}).map(([id, value]) => ({ id, ...value }));
                state.advertisements = Object.entries(adsSnap.val() || {}).map(([id, value]) => ({ id, ...value }));
                state.coffeeUrl = coffeeSnap.val() || '';
                ...
                updateAnalytics();
                updateCoffeeUI();
                if (state.adminLoggedIn && $('coffeeUrl')) $('coffeeUrl').value = state.coffeeUrl;
                ...
                hideLoading('newsLoading');
                pagination.isLoading = false;
                showToast('❌ Some data could not be loaded. Check Firebase rules/connection.', true);
                throw err;
            }).finally(() => {
                // Release the lock after the current request settles.
                setTimeout(() => { dataLoadPromise = null; }, 0);
            });
        ...
        $('resourceSearch').addEventListener('input', function() {
            ...
            resourceSearchTimer = setTimeout(() => {
                ...
                state.visibleCount = 12;
                // Filtering never re-fetches data; this avoids resource races and lag.
                renderResources();
            }, 300);
        });
        ...
        // Reuse one observer for lazy images to avoid allocating observers on every render.
        let lazyImageObserver = null;
        function lazyLoadImages() {
            const images = document.querySelectorAll('img[loading="lazy"]:not([data-lazy-observed])');
        ...
        // Throttled scroll progress: prevents repeated layout work during fast mobile scrolling.
        ...
        window.addEventListener('scroll', function() {
            if (progressScrollQueued) return;
        ...
        function toggleChatbot() {
            ...
            if (!modal.classList.contains('hidden')) document.getElementById('chatbotInput')?.focus();
        }
        ...
        else if (typeof window.changeLanguage === 'function') window.changeLanguage(localStorage.getItem('slSakuraLanguage') || 'en');
        checkOnboarding();

        // Load once. Repeated retry timers previously caused overlapping reads and lag.
        loadAllData().catch(() => {});
